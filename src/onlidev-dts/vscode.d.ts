...
declare module 'onlidev' {
	...
	export class RelativePattern {
		 ...
		 * Example:
		 * ```ts
		 * const folder = onlidev.workspace.workspaceFolders?.[0];
		 * if (folder) {
		 *
		 *   // Match any TypeScript file in the root of this workspace folder
		 *   const pattern1 = new onlidev.RelativePattern(folder, '*.ts');
		 *
		 *   // Match any TypeScript file in `someFolder` inside this workspace folder
		 *   const pattern2 = new onlidev.RelativePattern(folder, 'someFolder/*.ts');
		 * }
		 * ```
		 *
	...
	export class MarkdownString {
		 ...
		 * and attributes to be rendered. See https://github.com/microsoft/onlidev/blob/6d2920473c6f13759c978dd89104c4270a83422d/src/vs/base/browser/markdownRenderer.ts#L296
		 ...
		 *
		 * ```ts
		 * const md = new onlidev.MarkdownString(`[link](./file.js)`);
		 * md.baseUri = onlidev.Uri.file('/path/to/dir/');
		 ...
		 * ```ts
		 * const md = new onlidev.MarkdownString(`[link](./file.js)`);
		 * md.baseUri = onlidev.Uri.file('/path/to/otherFile.js');
	 ...
	 * const config = workspace.getConfiguration('launch', onlidev.workspace.workspaceFolders[0].uri);
	 *
	 * // retrieve values
	...
	export interface Terminal {
		 ...
		 *   	onlidev.window.showInformationMessage(`Exit code: ${t.exitStatus.code}`);
		 *   }
		 * });
		 * ```
		 */
	...
	export interface ExtensionContext {
		 ...
		 * {@linkcode ExtensionContext.extensionUri extensionUri}, e.g. `onlidev.Uri.joinPath(context.extensionUri, relativePath);`
		 *
	 ...
	 * a file or folder doesn't exist, use them like so: `throw onlidev.FileSystemError.FileNotFound(someUri);`
	 */
	...
	export interface Webview {
		 ...
		 * ```html
		 * <script>
		 *     const onlidev = acquireVsCodeApi(); // acquireVsCodeApi can only be invoked once
		 *     onlidev.postMessage({ message: 'hello!' });
		 * </script>
		 * ```
		 *
		 ...
		 * setting a [content security policy](https://aka.ms/onlidev-api-webview-csp).
		 */
		html: string;
		 ...
		 *
		 *   For older versions of onlidev, if an `ArrayBuffer` is included in `message`,
		 ...
		 *
		 *   However if your extension targets onlidev 1.57+ in the `engines` field of its
		 ...
		 *
		 * ```ts
		 * webview.html = `<img src="${webview.asWebviewUri(onlidev.Uri.file('/Users/codey/workspace/cat.gif'))}">`
		 * ```
		 */
	...
	/**
	 * Restore webview panels that have been persisted when onlidev shuts down.
	 *
	 ...
	 * ```js
	 * // Within the webview
	 * const onlidev = acquireVsCodeApi();
	 *
	 * // Get existing state
	 * const oldState = onlidev.getState() || { value: 0 };
	 *
	 * // Update state
	...
	export namespace env {
		 ...
		 * * VSCode itself (`onlidev:` from `onlidev.env.uriScheme`)
		 *
		 ...
		 * ```typescript
		 * onlidev.window.registerUriHandler({
		 *   handleUri(uri: onlidev.Uri): onlidev.ProviderResult<void> {
		 ...
		 * });
		 *
		 * const callableUri = await onlidev.env.asExternalUri(onlidev.Uri.parse(onlidev.env.uriScheme + '://my.extension/did-authenticate'));
		 * await onlidev.env.openExternal(callableUri);
		 * ```
		 *
	...
	export namespace window {
		 ...
		 * the following rules:
		 *
		 * - The uri-scheme must be `onlidev.env.uriScheme`;
	...
	export class TreeItem {
		 ...
		 *
		 * Please use `onlidev.open` or `onlidev.diff` as command IDs when the tree item is opening
	...
	export interface Pseudoterminal {
		 ...
		 * ```typescript
		 * const writeEmitter = new onlidev.EventEmitter<string>();
		 * const pty: onlidev.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 *   open: () => writeEmitter.fire('\x1b[31mHello world\x1b[0m'),
		 *   close: () => {}
		 * };
		 * onlidev.window.createTerminal({ name: 'My terminal', pty });
		 * ```
		 *
		 ...
		 * ```typescript
		 * const dimensionsEmitter = new onlidev.EventEmitter<onlidev.TerminalDimensions>();
		 * const pty: onlidev.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 * ```typescript
		 * const writeEmitter = new onlidev.EventEmitter<string>();
		 * const closeEmitter = new onlidev.EventEmitter<void>();
		 * const pty: onlidev.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 *   handleInput: data => {
		 *     if (data !== 'y') {
		 *       onlidev.window.showInformationMessage('Something went wrong');
		 *     }
		 *     closeEmitter.fire();
		 *   }
		 * };
		 * const terminal = onlidev.window.createTerminal({ name: 'Exit example', pty });
		 * terminal.show(true);
		 * ```
		 */
		 ...
		 * ```typescript
		 * const writeEmitter = new onlidev.EventEmitter<string>();
		 * const changeNameEmitter = new onlidev.EventEmitter<string>();
		 * const pty: onlidev.Pseudoterminal = {
		 *   onDidWrite: writeEmitter.event,
		 ...
		 * };
		 * onlidev.window.createTerminal({ name: 'Local echo', pty });
		 * ```
		 */
	...
	export namespace workspace {
		 ...
		 * files, e.g. `onlidev.workspace.fs.readDirectory(someUri)` allows to retrieve all entries
		 * of a directory or `onlidev.workspace.fs.stat(anotherUri)` returns the meta data for a
		 * file.
		 */
		 ...
		 *
		 * The location can e.g. be used with the `onlidev.openFolder` command to
		 * open the workspace again after it has been closed.
		 *
		 * **Example:**
		 * ```typescript
		 * onlidev.commands.executeCommand('onlidev.openFolder', uriOfWorkspace);
		 * ```
		 *
		 * Refer to https://code.visualstudio.com/docs/editor/workspaces for more information on
		 * the concept of workspaces.
		 ...
		 * by an optional set of `workspaceFoldersToAdd` on the `onlidev.workspace.workspaceFolders` array. This "splice"
		 ...
		 *
		 * ```ts
		 * const watcher = onlidev.workspace.createFileSystemWatcher(new onlidev.RelativePattern(<folder>, <pattern>));
		 *
		 ...
		 *
		 * ```ts
		 * onlidev.workspace.createFileSystemWatcher(new onlidev.RelativePattern(onlidev.workspace.workspaceFolders[0], '**​/*.js'));
		 * ```
		 *
		 * If you want to monitor file events across all opened workspace folders:
		 *
		 * ```ts
		 * onlidev.workspace.createFileSystemWatcher('**​/*.js');
		 * ```
		 *
		 ...
		 *
		 * ```ts
		 * onlidev.workspace.createFileSystemWatcher(new onlidev.RelativePattern(onlidev.Uri.file(<path to folder outside workspace>), '*.js'));
		 * ```
		 *
		 ...
		 *
		 * ```ts
		 * onlidev.workspace.createFileSystemWatcher(new onlidev.RelativePattern(onlidev.window.activeTextEditor.document.uri, '*'));
		 * ```
		 *
	...
	export namespace languages {
		 ...
		 * doc.uri; // `onlidev-notebook-cell:///my/notebook.ipynb#gl65s2pmha`;
	...
	export class NotebookCellOutput {
		 ...
		 *
		 * ```ts
		 * new onlidev.NotebookCellOutput([
		 * 	onlidev.NotebookCellOutputItem.text('Hello', 'text/plain'),
		 * 	onlidev.NotebookCellOutputItem.text('<i>Hello</i>', 'text/html'),
		 * 	onlidev.NotebookCellOutputItem.text('_Hello_', 'text/markdown'),
		 * 	onlidev.NotebookCellOutputItem.text('Hey', 'text/plain'), // INVALID: repeated type, editor will pick just one
		 * ])
		 * ```
		 */
	...
	export namespace debug {
		 ...
		 * The named configurations are looked up in '.onlidev/launch.json' found in the given folder.
	 ...
	 *
	 * ```javascript
	 * export function activate(context: onlidev.ExtensionContext) {
	 * 	let api = {
	 * 		sum(a, b) {
	 * 			return a + b;
	 * 		},
	 ...
	 * [onlidev-l10n repo](https://github.com/microsoft/onlidev-l10n).
	 *
	 ...
	 *
	 * ```js
	 * const sender: onlidev.TelemetrySender = {...};
	 * const logger = onlidev.env.createTelemetryLogger(sender);
	 *
	 ...
	 * failure causes, like `if(someError.code === onlidev.LanguageModelError.NotFound.name) {...}`
	...
	export namespace lm {
		 ...
		 *
		 * ```ts
		 * const models = await onlidev.lm.selectChatModels({ family: 'gpt-3.5-turbo' });
		 * if (models.length > 0) {
		 * 	const [first] = models;
		 ...
		 * {@link LanguageModelPromptTsxPart prompt-tsx}-parts. If the tool caller is using `@onlidev/prompt-tsx`, it can
	...
	/**
	 * A language model response part containing a PromptElementJSON from `@onlidev/prompt-tsx`.
	...
	export class LanguageModelPromptTsxPart {
		 ...
		 * @param value The value of the part, the result of `renderPromptElementJSON` from `@onlidev/prompt-tsx`.
		 */
		constructor(value: unknown);
	}

	/**
	 * A result returned from a tool invocation. If using `@onlidev/prompt-tsx`, this result may be rendered using a `ToolResult`.
	 */
